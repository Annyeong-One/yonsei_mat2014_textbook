...
```python
# Button click handlers (GUI programming)
...
### Multiple Expressions (Tuple Trick)

```python
# Execute multiple expressions (last one is returned)
...
# Find tuple with minimum second element
pairs = [(1, 5), (3, 2), (2, 8)]
...
# Regular function for documentation
...
```python
# Lambda in traceback shows as <lambda>
nums = [1, 2, 'three', 4]
# list(map(lambda x: x ** 2, nums))  # Unclear error message

# Named function gives better traceback
def square(x):
    return x ** 2
# list(map(square, nums))  # Clear error shows 'square' function
```

### 5. Reduced Readability for Complex Logic

```python
# Poor - complex lambda
result = list(filter(lambda x: x > 0 and x < 100 and x % 2 == 0 and x % 3 == 0, range(200)))

# Better - named function with clear intent
...
```python
# Poor - lambda assigned to non-descriptive name
f = lambda x: x ** 2

# Better - use descriptive name
square = lambda x: x ** 2

# Best - use regular function for named functions
def square(x):
    return x ** 2
```
...
### 4. Avoid Assigning Lambdas (PEP 8)
...
# Instead of: sorted(pairs, key=lambda p: p[1])
pairs = [(1, 5), (3, 2), (2, 8)]
...
```python
# Tkinter button handlers
import tkinter as tk

root = tk.Tk()

# Lambda captures variables in closure
for i in range(5):
